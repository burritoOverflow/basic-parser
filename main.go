...
// optional: clean up .o and .asm, leaving just the executable
var cleanUp = flag.Bool("cleanup", false, "Remove intermediate files (asm and object)")
...
// tryGenerateObjectFile attempts to generate an object file from the assembly code using NASM.
// outFilePath is the path to the assembly file generated by the compiler
// returns error on failure
func tryGenerateObjectFile(outFilePath string) error {
	...
	if *cleanUp {
		// remove the assembly file after generating the object file
		fmt.Printf("removing assembly file '%s'\n", outFilePath)
		err = os.Remove(outFilePath)
		if err != nil {
			fmt.Printf("Error removing assembly file '%s': %v\n", outFilePath, err)
			// not a big deal, the important stuff already was a success.
		}
	}
	return nil
}

// attempt to run gcc to link the object file and create an executable; returns err on failure
func tryRunGcc(outFilePath string) error {
	...
	// take the generated object file and link it to create an executable
	// we depend on libc for printf
	// use basename for the executable
	baseName := strings.TrimSuffix(outFilePath, filepath.Ext(outFilePath))
	objectFilePath := baseName + ".o"
	...
	if *cleanUp {
		// renmove the object file and after linking
		fmt.Printf("removing object file '%s'\n", objectFilePath)
		err = os.Remove(objectFilePath)
		if err != nil {
			fmt.Printf("Error removing object file '%s': %v\n", objectFilePath, err)
...
// takes the input file path ('source') and the output file path--the name of the various artifacts (assembly, object, executable)
func compileFileContents(inFilePath string, outFilePath string) {
	...
	// Generate the object file from the assembly code
	if err := tryGenerateObjectFile(outFilePath); err != nil {
		fmt.Printf("Error generating object file: %v\n", err)
		os.Exit(1)
	}

	// Link the object file to create an executable
	if err = tryRunGcc(outFilePath); err != nil {
		fmt.Printf("Error linking object file: %v\n", err)
		os.Exit(1)
	}
	...
	fileInfo, err := os.Stat(executableFilePath)
	if err != nil {
		fmt.Printf("Error getting stats for executable '%s': %v\n", executableFilePath, err)
		// Don't exit, linking was successful, not a big deal..
	} else {
		fmt.Printf("  Path: %s\n", executableFilePath)
